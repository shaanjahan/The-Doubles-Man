-- Tier-scaled hourly gameplay cap (economy tuning, 2026-07-31).
-- Exact deployed finalize_round_apply body with ONE change: v_cap is now
-- greatest(stored per-player cap, tier-scaled cap) — caps can only rise,
-- never fall, so existing testers are strictly grandfathered. No data is
-- read or written by this migration beyond replacing the function code.
...
begin
  ...
  v_cap := greatest(
    coalesce(nullif(v_player.hourly_earnings_cap, 0), 3500),
    -- Tier-scaled gameplay cap: late tiers earn faster so free play stays
    -- rewarding; greatest() with the stored per-player cap means nobody's
    -- cap ever DECREASES (tester grandfathering guarantee).
    case greatest(coalesce(v_player.business_tier, 0), 0)
      when 0 then 3500 when 1 then 5000 when 2 then 6500
      when 3 then 8000 when 4 then 12000 when 5 then 18000
      else 25000
    end);
